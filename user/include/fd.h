...
// 设备结构体，设置了相应的设备id和对设备的基本操作，通过函数指针实现
// MOS中共有三种设备类型：文件、控制台和管道
struct Dev {
	int dev_id; // 设备id
	char *dev_name; // 设备名称
...
// 通过指针的强制转换改变了对地址处数据的解释方式，并不改变指针的值
// Filefd的第一个成员就是Fd，因此指向Filefd的指针同样指向这个Fd的起始位置，故可以进行强制转换。
struct Filefd {
	struct Fd f_fd;
	u_int f_fileid;
	struct File f_file;
