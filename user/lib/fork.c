...
static void __attribute__((noreturn)) cow_entry(struct Trapframe *tf) {
	u_int va = tf->cp0_badvaddr; //发生页写入异常的地址
	u_int perm;
...
static void duppage(u_int envid, u_int vpn) {
	...
	if ((perm & PTE_D) && !(perm & PTE_LIBRARY)) {
		perm = (perm & ~PTE_D) | PTE_COW; //修改权限位，取消PTE_D，加上PTE_COW，实现写时复制
		syscall_mem_map(0, addr, envid, addr, perm); // 将该页面映射给子进程
		syscall_mem_map(0, addr, 0, addr, perm); //父进程映射给自己是为了修改自己的权限位
	} else {
		// 对于只读页面，直接由父进程映射给子进程
